...
class EventHandler(pyinotify.ProcessEvent):
    ...
    def process_IN_CREATE(self, event):
        ...
        if os.path.isfile(event.pathname):
            ...
            if self.log_count % self.log_rate == 0:
                print "added %s to queue to add: " % [event.pathname, str(datetime.now())]
        else:
            
            print "the path %s added to fs is not file" % [event.pathname, str(datetime.now())]
    ...
    def process_IN_DELETE(self, event):
        ...
        if self.log_count % self.log_rate == 0:
            print "added %s to queue to delete: " % [event.pathname, str(datetime.now())]
        self.log_count += 1
